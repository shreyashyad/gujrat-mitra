...
function SideCardWithImage({ article }) {
  ...
  return (
    <div
      ...
      className="group relative flex h-full flex-row md:flex-col justify-between cursor-pointer gap-3 md:gap-0
    ...
    >
      ...
      <div className="flex flex-1 flex-col justify-between min-w-0 gap-1">
        <p className="mt-0 md:mt-1.5 article-headline line-clamp-3">
          {article.headline}
        </p>

        <div className="article-metaRow md:mt-auto">
...
function SideCardTextOnly({ article }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col justify-between min-w-0 gap-1 md:min-h-[110px]">
        <div className="relative pr-0 md:pr-8">
          <p className="article-headline line-clamp-3">
            {article.headline}
          </p>

          <div className="article-metaRow md:mt-auto flex items-center justify-between">
...
function BottomCard({ article }) {
  ...
  return (
    <div
      ...
      onClick={() => openNews(article)}
      onKeyDown={(e) => e.key === "Enter" && openNews(article)}
      className="group flex h-full flex-col justify-between cursor-pointer pt-3
                 hover:-translate-y-0.5 transition-all duration-300 ease-out active:scale-[0.985]"
    >
      <div className="min-w-0 flex-1 flex flex-col">
        ...
        <p className="mt-2 article-headline line-clamp-3">
          {article.headline}
        </p>
      </div>
...
function ListCard({ article }) {
  ...
  return (
    <div
      ...
      onClick={() => openNews(article)}
      onKeyDown={(e) => e.key === "Enter" && openNews(article)}
      className="group relative flex h-full cursor-pointer gap-3
                 flex-row md:flex-col
                 justify-between
                 md:pt-3
                 hover:-translate-y-0.5 transition-all duration-300 ease-out active:scale-[0.985]"
    >
      {/* Image: fixed size on mobile, full aspect on desktop (768px+) */}
      <div className="relative w-24 sm:w-28 h-[86px] md:h-auto shrink-0 md:w-full md:aspect-[14/8] overflow-hidden rounded-lg">
        {article.img ? (
          <img
            src={article.img}
            alt={article.headline || ""}
        ...
        <button
          ...
          onClick={handleBookmark}
          title={isBookmarked ? "સેવ થયેલ છે" : "સેવ કરો"}
          className={`absolute top-1.5 right-1.5 md:top-2 md:right-2 flex h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 items-center justify-center rounded-full backdrop-blur-md transition-all duration-200 cursor-pointer z-10 ${
            isBookmarked
              ? "bg-[#e48d0b] text-white dark:bg-[#e6c27a] dark:text-black"
              : "bg-black/40 text-white hover:bg-black/60"
          }`}
        >
          <Bookmark
            size={14}
            className={`md:w-4 md:h-4 ${isBookmarked ? "fill-current" : ""}`}
          />
        </button>
      </div>

      {/* Text */}
      <div className="flex flex-1 flex-col justify-between min-w-0 gap-1 md:mt-2">
        <p className="article-headline line-clamp-3">
          {article.headline}
        </p>
...
function SkeletonFeed() {
  return (
    <div className="space-y-5 sm:space-y-6 lg:space-y-8 animate-pulse">
      <div className="grid grid-cols-1 gap-3 sm:gap-3.5 md:grid-cols-[1fr_1.15fr] md:gap-4">
...
export default function CategoryFeed({ dataKey, slug, categoryLabel }) {
   ...
   * Desktop (≥768, ONE system):
   *   always 4-col + vertical cards (ListCard is vertical from md:)
   *
  ...
  const getRowSectionClass = (rowIndex) => {
    ...
    if (isGridOnMobile) {
      return `grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-5 ${desktopSectionStyles}`;
    }

    return `grid grid-cols-1 gap-4 md:grid-cols-4 md:gap-5 ${desktopSectionStyles}`;
  };

  return (
    <div className="space-y-6 lg:space-y-8">
      {/* ========== TOP BLOCK (hero + side) — ONE desktop structure from 768px+ ========== */}
      <section
        className="grid grid-cols-1 gap-6 md:grid-cols-[1fr_1.15fr] md:items-stretch
      ...
      >
        ...
        <div className="flex h-full flex-col items-stretch gap-1">
          {row1.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 items-stretch gap-2 md:gap-6">
              {row1.map((article, i) => (
          ...
          <div className="flex flex-1 flex-col items-stretch gap-1 mt-1">
            {textRow1.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 items-stretch gap-2 md:gap-6">
