...
function HeroCard({ article }) {
  ...
  return (
    <div
      ...
      className="group flex h-full flex-col justify-between md:min-h-[340px] cursor-pointer p-0 transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.985]"
    >
...
function SmallCard({ article }) {
  ...
  return (
    <div
      ...
      onClick={() => openNews(article)}
      onKeyDown={(e) => e.key === "Enter" && openNews(article)}
      className="group relative flex w-full h-full flex-1 items-stretch gap-3 cursor-pointer p-0 md:min-h-[102px] transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.985]"
    >
      {/* Thumbnail */}
  ...
  return (
    <section className="w-full pt-3 pb-7">
      <div className="mx-auto max-w-8xl pt-3">
        {/* Outer grid: 1 col on mobile, 2 cols on desktop (768px+, ONE system) */}
