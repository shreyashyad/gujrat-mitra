...
function FeaturedCard({ article }) {
  ...
  return (
    <div
      ...
      className="group flex h-full flex-col justify-between md:min-h-[336px] cursor-pointer transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.985]"
    >
...
function MidCard({ article, className = "" }) {
  ...
  return (
    <div
      ...
      onClick={() => openNews(article)}
      onKeyDown={(e) => e.key === "Enter" && openNews(article)}
      className={`group flex h-full w-full items-stretch gap-3 cursor-pointer md:min-h-[102px] transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.985] ${className}`}
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between py-0.5">
        <p className="article-headline leading-snug line-clamp-3">
          {article.headline}
        </p>
...
function TextCard({ article, className = "" }) {
  ...
  return (
    <div
      ...
      onClick={() => openNews(article)}
      onKeyDown={(e) => e.key === "Enter" && openNews(article)}
      className={`group relative flex w-full h-full flex-col justify-between md:min-h-[102px] cursor-pointer transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.985]
        ${className}`}
    >
      {/* Content */}
...
export default function MaruShaher() {
  ...
  return (
    <section className="md:items-stretch shadow-[0_2px_3px_rgba(0,0,0,0.06),0_0_20px_rgba(0,0,0,0.06)] p-7 bg-white dark:bg-[#121212] border border-gray-200/70 dark:border-white/10 rounded-3xl">
      ...
      */}
      <div className="grid grid-cols-1 md:grid-cols-[1.2fr_1.2fr_1.1fr] gap-6 md:gap-5 md:items-stretch">
        {/* Featured */}
        <div>
