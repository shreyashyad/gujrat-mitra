...
function FeaturedCard({ article }) {
  ...
  return (
    <div
      ...
      className="group flex h-full flex-col justify-between cursor-pointer transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.985]"
    >
...
function MidCard({ article, className = "" }) {
  ...
  return (
    <div
      ...
      onClick={() => openNews(article)}
      onKeyDown={(e) => e.key === "Enter" && openNews(article)}
      className={`group flex w-full h-full items-stretch gap-3 cursor-pointer md:min-h-[102px] transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.985] ${className}`}
    >
...
export default function MaruGujarat() {
  ...
  return (
    <section className="lg:items-stretch shadow-[0_2px_3px_rgba(0,0,0,0.06),0_0_20px_rgba(0,0,0,0.06)] p-7 bg-white dark:bg-[#121212] border border-gray-200/70 dark:border-white/10 rounded-3xl">
      ...
      {/*
        Mobile (<768) → 1 column
        Desktop (768px+, ONE system) → 3-column, fluid widths
      */}
      <div className="grid grid-cols-1 md:grid-cols-[1.2fr_1.2fr_1.1fr] gap-3 md:gap-5 md:items-stretch">
        {/* Featured */}
        <div>
