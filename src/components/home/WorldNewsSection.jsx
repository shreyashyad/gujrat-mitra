...
function ImageCard({ article }) {
  ...
  return (
    <div
      ...
      className="group flex h-full flex-col justify-between md:min-h-[343px] cursor-pointer p-0 transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.985]"
    >
...
/** Text card – pure text-only vertical at ALL widths */
function TextCard({ article, className = "" }) {
  ...
  return (
    <div
      ...
      onClick={() => openNews(article)}
      onKeyDown={(e) => e.key === "Enter" && openNews(article)}
      className={`group relative flex h-full w-full cursor-pointer transition-transform duration-300 ease-out hover:-translate-y-0.5 active:scale-[0.985]
        flex-col justify-between md:min-h-[102px]
        ${className}`}
    >
      {/* Content */}
...
export default function TopNewsSection() {
  ...
  return (
    <section className="md:items-stretch shadow-[0_2px_3px_rgba(0,0,0,0.06),0_0_20px_rgba(0,0,0,0.06)] p-7 lg:pt-7 lg:px-7 lg:pb-2 bg-white dark:bg-[#121212] border border-gray-200/70 dark:border-white/10 rounded-3xl">
      {/* 
        Mobile (<768) : 1 column
        md+ (≥768)     : single 3-column desktop layout
      */}
